module 747InductionTutorial where
...
_ : (3 + 4) + 5 ≡ 3 + (4 + 5)  -- here proving associativity. 
_ =
  begin
    (3 + 4) + 5
  ≡⟨⟩
...
+-identityᴸ : ∀ (m : ℕ) → zero + m ≡ m  -- proving some properties of natural numbers. Left identity of plus
+-identityᴸ m = refl -- is true because plus defined on left recusion of argument. Just true
...
{-
+-identityʳ : ∀ (m : ℕ) → m + zero ≡ m -- When we see a goal where two sides have different shapes we need goal to reduce. When all else fails, induction.
+-identityʳ zero = refl -- here need to super reduce to show 0 ≡ 0
+-identityʳ (suc m)̂ rewrite +-identityʳ m = refl -- type as +identity backshalsh hat r 4 (4th option) ʳ  . So this rerite does something here
-}
...
-- λ is type  G l . Mnemonic: G is Gree. μM T θ ε ≣
+-assoc : ∀ (m n p : ℕ) → (m + n) + p ≡ m + (n + p)
+-assoc zero n p = refl  -- here base case is 0 + n + p = 0 + n + p
+-assoc (suc m) n p = cong suc (+-assoc m n p)  -- case split here
...
+-suc zero n = refl
+-suc (suc m) n = cong suc (+-suc m n)  -- plus defined on left recursion show should do it on left side
-- so here I think cong is using a definition here

-- Commutativity of addition.
-- Equational proof shown in PLFA.

{-
-- so previous +- suc lemma usefull for this one
+-comm : ∀ (m n : ℕ) → m + n ≡ n + m
+-comm zero n = sym (+-identityʳ′ n)
+-comm (suc m) n = begin  -- lets do induction on m. Could do induction on m and n but lets stick to n.
...
-- 747/PLFA exercise: MultComm (3 points)  Worth more because need a helper lemma along the way
