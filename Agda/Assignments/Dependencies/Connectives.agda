...
module Dependencies.Connectives where
...
open import Dependencies.Isomorphism using (extensionality; _≃_; _≲_; _⇔_)
...
⊤-identityʳ : ∀ {A : Set} → (A × ⊤) ≃ A
⊤-identityʳ = Dependencies.Isomorphism.mk-≃
  proj₁
  (λ a → ⟨ a , tt ⟩)
...
→-distrib-⊎ {A} {B} {C} = Dependencies.Isomorphism.mk-≃ to′ from′ from∘to′ to∘from′
  where
    to′ :  (A ⊎ B → C) → (A → C) × (B → C)
