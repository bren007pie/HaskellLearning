...
-- hmm, so recursion happens b.c. sucᶜ is a lambda function pointing to `suc
  -- which then recurses down until we get term substitution
_ : (sucᶜ · (sucᶜ · ` "z")) [ "z" := `zero ] ≡  sucᶜ · (sucᶜ · `zero)
_ = refl

-- here we got application and term substituion 
_ : (ƛ "z" ⇒ ` "s" · (` "s" · ` "z")) [ "s" := sucᶜ ] ≡  ƛ "z" ⇒ sucᶜ · (sucᶜ · ` "z")
_ = refl

-- here there's a layer of recursion to sub in N [ "y" := `zero ]
_ : (ƛ "x" ⇒ ` "y") [ "y" := `zero ] ≡ ƛ "x" ⇒ `zero 
_ = refl

-- here x is equal to y (x ≟ y yes) so no substitution
_ : (ƛ "x" ⇒ ` "x") [ "x" := `zero ] ≡ ƛ "x" ⇒ ` "x"
_ = refl

-- nothing equal and no x≟y so just stays itself
_ : (ƛ "y" ⇒ ` "y") [ "x" := `zero ] ≡ ƛ "y" ⇒ ` "y"
_ = refl

-- let's try making 2 and doing substitution for zero
