...
-- Tips from class:
-- Use implement bool using built-in
...
data _⊢_ : Context → Type → Set where
  ...
  case⊥ : ∀ {Γ A} → Γ ⊢ `⊥ → Γ ⊢ A  -- aka ⊥-E/bottom elimination
...
rename p (case⊥ L)      =  case⊥ (rename p L) -- HID: followed similar logic to case×
...
progress (case⊥ L) with progress L  -- following same form ase case×
...    | step L—→L′                         =  {!!}
...    | done (VL)                           =  {!!}
