module 747NaturalsTutorial where
...
import Relation.Binary.PropositionalEquality as Eq  -- Importing package, giving it short name
...
private  -- Private block this stuff won't be exported outside the file, can add private tests
  _ : 2 + 3 ≡ 5
  _ = ?

-- Equational reasoning. This is a whole proof written out here.
_ : 2 + 3 ≡ 5
_ =
  begin
  2 + 3
  ≡⟨⟩    -- is shorthand for -- typed as ( \ == ) ( | <) (\ >)
            -- \==<> is another form of 'just true' when lhs = rhs is true by definition of computation
...
_*_ : ℕ → ℕ → ℕ
zero * n = zero
suc m * n = m * n + n  --(m+1) * n = ???? expanded = m*n + 1*n = m*n+n
...
-- {-# BUILTIN NATPLUS _+_ #-}  -- Binding to buildins here so we can use normal notation defined
...
data Bin-ℕ : Set where   -- numbers using blocks of bits
  bits : Bin-ℕ 
  _x0 : Bin-ℕ → Bin-ℕ
  _x1 : Bin-ℕ → Bin-ℕ  -- taking arguments on the left

-- Our representation of zero is different from PLFA.
-- We use the empty sequence of bits (more consistent).

bin-zero : Bin-ℕ
bin-zero = bits  -- empty sequency 

bin-one : Bin-ℕ
bin-one = bits x1     -- 1 in binary

bin-two : Bin-ℕ
bin-two = bits x1 x0  -- 10 in binary, written in right away

-- 747 exercise: Increment (1 point)  -- Do this exercise
