module 747NegationTutorial where
...
¬_ : Set → Set
¬ A = A → ⊥  -- Type of not A is the type that returns false. ¬ is typed \ n e g

-- if both ¬ A and A hold, then ⊥ holds (not surprisingly).

¬-elim : ∀ {A : Set} → ¬ A → A → ⊥  -- this is top
¬-elim ¬a a = ¬a a
...
¬¬¬-elim : ∀ {A : Set} → ¬ ¬ ¬ A → ¬ A
¬¬¬-elim a‴ a = a‴ (¬¬-intro a)  -- two ways of getting a ⊥. Either a‴ or elimination
...
-- not-equal-to. It's just not equal. ≢ is \ = = n
...
peano = λ {()}  -- zero isn't suc of anything. Just true.
...
id : ⊥ → ⊥   --Carette published a paper where this one works and id′ does not.
id x = x  -- identity function is lazy, doesn't look at the argument

id′ : ⊥ → ⊥
id′ ()  -- this one is eager, takes argument?

id≡id′ : id ≡ id′
id≡id′ = extensionality (λ())  --avoid extensionality
